...
export function SocialLoginButtons({
  ...
  if (clientEnv.VITE_ALLOW_GITHUB_AUTH) {
    buttons.push(
      <Button
        key="github"
        type="button"
  ...
  if (clientEnv.VITE_ALLOW_GOOGLE_AUTH) {
    buttons.push(
      <Button
        key="google"
        type="button"
    ...
    buttons.push(
      ...
      >
        {isLoading ? (
          ...
          <FeishuIcon className="mr-2 h-5 w-5" />
        )}
        Feishu
      </Button>,
    );
  ...
  return <div className="grid gap-3">{buttons}</div>;
}

// Divider component
export function AuthDivider() {
  const { t } = useTranslation();
  const hasPassword = clientEnv.VITE_ALLOW_PASSWORD_AUTH;
  const hasSocial =
    clientEnv.VITE_ALLOW_GITHUB_AUTH ||
    clientEnv.VITE_ALLOW_GOOGLE_AUTH ||
    clientEnv.VITE_ALLOW_FEISHU_AUTH;

  if (!hasPassword || !hasSocial) return null;
